...
public class Graph {
    ...
    private void init(int size) {	
        ...
        for(int i = 0; i < edges; i++) {            
            nodes.get(rawData[i][0] - 1).addChild(nodes.get(rawData[i][1] - 1));
            nodes.get(rawData[i][0] - 1).addChild(nodes.get(rawData[i][1] - 1));
        }
    ...
    private void setProperties() {
        ...
        //acyclic = (removed.size() == nodes.size());
        acyclic = (nodes.size() == 0 && removed.size() != 0);
    ...
    public void reduce() {
        ...
        do {
            changes = false;
            for(int i = 0; i < nodes.size(); i++) {
                if(nodes.get(i).getDegree() <= 1){
                    removed.add(nodes.remove(i));
                    removed.getLast().remove();
                    changes = true;
                }
            }
        }while(changes);
    ...
    private void findSubgraphs() {
        if(size != 0) {
            boolean[] mask = new boolean[nodes.getLast().getId() + 1];	//false: not done, true: done
            int done = 0;	
            ...
            do{
                ...
                LinkedList<Node> group = findRelatives(nodes.get(i), mask);
                group.add(nodes.get(i));
                done += group.size();
    ...
    private LinkedList<Node> findRelatives(Node n, boolean[] mask) {
        ...
        for(Node child : n.getChildren()) {
            if(!mask[child.getId()]){			//if not done yet
                ...
                relatives.add(child);
                relatives.addAll(findRelatives(child, mask));   //TODO exception on graph05
            }
        }
        return relatives;
    }
     ...
     */
    public LinkedList<Node> getRemoved() {
        return removed;
    }
    public void sort() {
