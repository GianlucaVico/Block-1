...
public class UpperBound implements Solver {
    ...
    private int getBound(Graph sub) {
        ...
        if(sub.isTrivial()) {
            ...
            LinkedList<Node> differentColor = new  LinkedList<Node>(sub.getNodes());	//nodes with a with a different color from the current node                        
            LinkedList<Node> sameColor = new LinkedList<Node>();	//nodes with the same color of the current one
            LinkedList<Node> checked = new LinkedList<Node>();		//node already checked (to avoid infinite loops)
            ...
            while(!done) {
                ...
                //check end condition
                if(differentColor.size() < 1) {
                    done = true;
                }else {
                    Node tmpStart = null;
                    for(Node child: start.getChildren()){
                        if((tmpStart == null || child.getDegree() > tmpStart.getDegree()) && !checked.contains(child))
                            tmpStart = child;
                    }
                    if(tmpStart == null){
                        int i = 0;
